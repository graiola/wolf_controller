...
public:
    ...
    JoyHandler(ros::NodeHandle& node, wolf_controller::ControllerCore* controller_ptr, const std::string& topic = "joy")
        :DeviceHandlerRosInterface(node,controller_ptr,topic)
    {
        // Set handlers
        if(controller_ptr!=nullptr)
        {
            switch_posture_.f_ = boost::bind(&wolf_controller::ControllerCore::switchPosture,controller_ptr);
            switch_control_mode_.f_ = boost::bind(&wolf_controller::ControllerCore::switchControlMode,controller_ptr);
            emergency_stop_.f_ = boost::bind(&wolf_controller::ControllerCore::emergencyStop,controller_ptr);
            reset_base_.f_ = boost::bind(&wolf_controller::ControllerCore::resetBase,controller_ptr);
        }
        else
            throw std::runtime_error("Controller not initialized yet!");

        if(controller_ptr->getGaitGenerator()!=nullptr)
            switch_gait_.f_ = boost::bind(&wolf_controller::ControllerCore::switchGait,controller_ptr);
        else
...
public:
   ...
   Ps3JoyHandler(ros::NodeHandle& node, wolf_controller::ControllerCore* controller_ptr, const std::string& topic = "joy")
...
public:
    ...
    XboxJoyHandler(ros::NodeHandle& node, wolf_controller::ControllerCore* controller_ptr, const std::string& topic = "joy")
         :JoyHandler(node,controller_ptr,topic)
    {
    }
...
public:
    ...
    SpaceJoyHandler(ros::NodeHandle& node, wolf_controller::ControllerCore* controller_ptr, const std::string& topic = "/spacenav/joy")
         :JoyHandler(node,controller_ptr,topic)
    {
        th = 0.5;
    }
