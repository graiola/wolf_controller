...
class JoyHandler : public DeviceHandlerRosInterface<sensor_msgs::msg::Joy>
{
...
public:
        ...
        : DeviceHandlerRosInterface(node, controller_ptr, topic)
    {
        ...
        {
            switch_posture_.f_ = std::bind(&wolf_controller::ControllerCore::switchPosture, controller_ptr);
            switch_control_mode_.f_ = std::bind(&wolf_controller::ControllerCore::switchControlMode, controller_ptr);
            emergency_stop_.f_ = std::bind(&wolf_controller::ControllerCore::emergencyStop, controller_ptr);
            reset_base_.f_ = std::bind(&wolf_controller::ControllerCore::resetBase, controller_ptr);
        }
        else
            throw std::runtime_error("Controller not initialized yet!");

        if(controller_ptr->getGaitGenerator() != nullptr)
            switch_gait_.f_ = std::bind(&wolf_controller::ControllerCore::switchGait, controller_ptr);
        else
    ...
    virtual void cmdCallback(const std::shared_ptr<sensor_msgs::msg::Joy> msg) = 0;
...
public:
        ...
        : JoyHandler(node, controller_ptr, topic)
   {
    ...
    virtual void cmdCallback(const sensor_msgs::msg::Joy::SharedPtr msg)
    {
...
public:
    ...
    virtual void cmdCallback(const std::shared_ptr<sensor_msgs::msg::Joy> msg)
    {
        if(msg.get() && !msg->axes.empty() && !msg->buttons.empty())
        {
            base_velocity_y_scale_     = static_cast<double>(msg->axes[0]);
